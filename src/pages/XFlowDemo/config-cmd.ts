  ...
  config.setRegisterHookFn((hooks) => {
    const list = [
      hooks.saveGraphData.registerHook({
        ...
        handler: async (args) => {
          if (!args.saveGraphDataService) {
            args.saveGraphDataService = ActionService.saveGraphData;
          }
        },
      }),
      hooks.addNode.registerHook({
        ...
        handler: async (args) => {
          args.createNodeService = ActionService.addNode;
        },
      }),
      ...
      hooks.addEdge.registerHook({
        ...
        handler: async (args) => {
          args.createEdgeService = ActionService.addEdge;
          args.edgeConfig = {
            ...args.edgeConfig,
  ...
  app.executeCommandPipeline([
    /** 1. 从服务端获取数据 */
    {
      ...
      getCommandOption: async () => {
        return {
          args: {
            loadDataService: ActionService.loadGraphData,
          },
        };
      },
